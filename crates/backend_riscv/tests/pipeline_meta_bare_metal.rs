...
#[test]
fn pipeline_meta_bare_metal() -> Result<()> {
    ...
    // Load RISC-V bare_metal manifest
    let manifest_path = ws_root.join("targets").join("riscv32imac_bare.toml");
    ...
    // README.txt should include metadata lines (appended when metadata is present)
    ...
    if readme.is_empty() {
        ...
        panic!("README.txt missing or empty for bare_metal.\nWARN.txt:\n{warn}");
    }

    // Presence-only checks (values not strictly asserted)
    assert!(readme.contains("size_optimized="), "README missing size_optimized=: \n{readme}");
    assert!(readme.contains("use_compressed="), "README missing use_compressed=: \n{readme}");
