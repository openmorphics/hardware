...
#[test]
fn pipeline_meta_linux_user() -> Result<()> {
    ...
    // Load RISC-V linux_user manifest
    let manifest_path = ws_root.join("targets").join("riscv64gcv_linux.toml");
    ...
    // README.txt should include metadata lines
    ...
    if readme.is_empty() {
        ...
        panic!("README.txt missing or empty for linux_user.\nWARN.txt:\n{warn}");
    }

    // Presence-only checks (values not strictly asserted)
    assert!(readme.contains("align_bytes="), "README missing align_bytes=: \n{readme}");
    assert!(readme.contains("quant_bits_default="), "README missing quant_bits_default=: \n{readme}");
    assert!(readme.contains("vector_available="), "README missing vector_available=: \n{readme}");
